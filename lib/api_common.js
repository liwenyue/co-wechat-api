...
/**
  ...
  async getAppWechatAuthorizeURL ({redirectUri, authType = 3, appId}) {
    ...
    let redirectUrl = `https://mp.weixin.qq.com/safe/bindcomponent?action=bindcomponent&component_appid=${this.component_appid}&pre_auth_code=${pre_auth_code}&redirect_uri=${redirectUri}`;
    if (appId) {
      redirectUrl = `${redirectUrl}&biz_appid=${appId}`;
    } else {
      redirectUrl = `${redirectUrl}&auth_type=${authType}`;
    }
    return redirectUrl;
  }
