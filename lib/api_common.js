...
var UserAccessToken = function (data) {
 ...
 */
var API = function (componentAppid, componentAppSecret, authorizerAppid, componentVerifyTicket, getComponentTicket, getComponentToken, saveComponentToken, getToken, saveToken, getUserToken, saveUserToken, authorizerRefreshToken) {
...
API.prototype.getAuthorizationInfo = function* (authCode) {
  ...
  if (data && data['authorization_info']) {
    ...
    var expireTime = (new Date().getTime()) + (authInfo.expires_in - 10) * 1000;
    var accessToken = new AccessToken(authInfo.authorizer_access_token, expireTime);
...
API.prototype.getAuthorizeURL = function (redirect, state, scope) {
  return `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${this.appid}&component_appid=${this.component_appid}&response_type=code&scop=${'snsapi_base' || scope}&state=${state}&redirect_uri=${redirect}#wechat_redirect`;
};
...
API.prototype.getAuthorizeURLForWebsite = function (redirect, state, scope) {
  return `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${this.appid}&component_appid=${this.component_appid}&response_type=code&scop=${'snsapi_login' || scope}&state=${state}&redirect_uri=${redirect}#wechat_redirect`;
};
...
API.prototype.getUserAccessToken = function* (code) {
  let componentAccessToken = yield this.ensureComponentToken();
  let url = `https://api.weixin.qq.com/sns/oauth2/component/access_token?appid=${this.app_id}&code=${code}&grant_type=authorization_code&component_appid=${this.component_appid}&component_access_token=${componentAccessToken.componentAccessToken}`;
  let args = {
    method: 'post',
    data: JSON.stringify({}),
    timeout: 10000,
  ...
  let token = yield this.request(url, args);
  token.create_at = new Date().getTime();
  yield this.saveUserToken(token.openid, token);
  return token;
};
...
API.prototype.refreshUserAccessToken = function* (refreshToken) {
  let componentAccessToken = yield this.ensureComponentToken();
  let url = `https://api.weixin.qq.com/sns/oauth2/refresh_token?appid=${this.app_id}&refresh_token=${refreshToken}&grant_type=refresh_token&component_appid=${this.component_appid}&component_access_token=${componentAccessToken.componentAccessToken}`;
  let args = {
    method: 'post',
    data: JSON.stringify({}),
    timeout: 10000,
...
API.prototype._getUser = function *(options, accessToken) {
  var url = `https://api.weixin.qq.com/sns/userinfo?access_token=${accessToken}&openid=${options && options.openid || options}&lang=${options && options.lang || 'zh_CN'}`;
  let args = {
    method: 'post',
    data: JSON.stringify({}),
    timeout: 10000,
...
API.prototype.getUser = function *(options) {
  ...
  let token = yield this.getUserAccessToken(options.openid);
  if (!token) {
