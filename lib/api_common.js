...
API.prototype.getAuthorizationInfo = function* (authCode, saveToken = false) {
  let componentAccessToken = yield this.ensureComponentToken();
  ...
  let data = yield this.request(url, args);
  try {
    if (data && data['authorization_info'] && saveToken) {
...
API.prototype.ensureAccessToken = function* () {
  ...
  if (accessToken && (accessToken = AccessToken(accessToken.accessToken, accessToken.expireTime)).isValid()) {
    return accessToken;
  }
