...
class API {
   ...
   * async () {
   *   let ticket = yield redis.get(`${config.open.appId}:ComponentVerifyTicket`,config.wxCacheDb);
   *   //console.log(`get ${config.open.appId}:ComponentVerifyTicket`,  ticket );
   *   return ticket;
   * },
   * async (){
   *   let result = yield redis.get(`${config.open.appId}:componentAccessToken`,config.wxCacheDb);
   *   //console.log(`get componentAccessToken`,  result);
   *   return JSON.parse(result);
   * },
   * async (token) {
   *   //console.log(`save componentAccessToken`,  token);
   *   yield redis.set(`${config.open.appId}:componentAccessToken`,JSON.stringify(token),config.wxCacheDb);
   * },
   * async (){
   *   let accessToken = yield redis.get(`${appId}:accessToken`);
   *   let refreshToken = yield redis.get(`${appId}:refreshToken`);
   ...
   *   yield redis.set(`${appId}:accessToken`,JSON.stringify(accessToken));
   *   yield redis.set(`${appId}:refreshToken`,refreshToken);
   * });
   * 
   * async getTicketToken() {
   *   // 传入一个获取全局token的方法
   *   let result = yield redis.get(`${appId}:ticket`,config.wxCacheDb);
   *   return JSON.parse(result);
   * }
   *
   * async saveTicketToken(token) {
   *   yield redis.set(`${appId}:ticket`,JSON.stringify(token),config.wxCacheDb);
   * }
   ...
   */
  constructor(componentAppid, componentAppSecret, authorizerAppid, getComponentTicket, getComponentToken, saveComponentToken, getToken, saveToken) {
    this.component_appid = componentAppid;
    this.component_appsecret = componentAppSecret;
    this.appid = authorizerAppid;
    this.getComponentTicket = getComponentTicket || async function (componentTicket) {
      this.componentTicket = componentTicket;
      return this.componentTicket;
    };
    this.getComponentToken = getComponentToken || async function () {
      return this.componentTokenStore;
    };
    this.saveComponentToken = saveComponentToken || async function (componentToken) {
  ...
  async getAccessToken() {
    ...
    await this.saveToken(token, data.refreshToken);
    return token;
  }
   ...
   * @param  redirect_url 
   */
  async getAppAuthorizeURL (redirect_url) {
    let {pre_auth_code} = await this.getPerAuthCode();
    if (!pre_auth_code) {
   ...
   */
  async getAuthorizationInfo (authCode, isSaveToken = false) {
    let componentAccessToken = await this.ensureComponentToken();
    ...
    let data = await this.request(url, args);
    if (data && data['authorization_info'] && isSaveToken) {
      let {authorizer_access_token, expires_in, authorizer_refresh_token} = data['authorization_info'];
      let expireTime = (new Date().getTime()) + (expires_in - 10) * 1000;
      let accessToken = new AccessToken(authorizer_access_token, expireTime);
      await this.saveToken(accessToken, authorizer_refresh_token);
    }
    return data;
  }
   ...
   */
  async getAuthorizerInfo (saveToken) {
    let componentAccessToken = await this.ensureComponentToken();
