...
class API {
   ...
   */
  constructor(componentAppid, componentAppSecret, authorizerAppid, authorizerRefreshToken, componentVerifyTicket, getComponentToken, saveComponentToken, getToken, saveToken) {
  ...
  async getAccessToken() {
    var componentAccessToken = await this.getComponentAccessToken();
    ...
    var token = new AccessToken(data.authorizer_access_token, expireTime);
  ...
  async ensureComponentToken() {
    var component_access_token = await this.getComponentToken();
    var accessToken;
