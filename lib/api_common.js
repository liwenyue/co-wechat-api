...
class Oauth {
  ...
  async getUserAccessToken (code) {
    let componentAccessToken = await this.ensureComponentToken();
    // let url = `https://api.weixin.qq.com/sns/oauth2/component/access_token?appid=${this.appId}&code=${code}&grant_type=authorization_code&component_appid=${this.componentAppId}&component_access_token=${componentAccessToken.componentAccessToken}`;
    let url = this.getRequestUrl('https://api.weixin.qq.com/sns/oauth2/refresh_token', {
      appid: this.appId,
      code: code,
      grant_type: 'authorization_code',
      component_appid: this.componentApi,
  ...
  async getSessionKey (code) {
    let componentAccessToken = await this.ensureComponentToken();
    let url = this.getRequestUrl('https://api.weixin.qq.com/sns/jscode2session', {
      appid: this.appId,
      secret: this.appsecret,
      js_code: code,
      grant_type: 'authorization_code',
  ...
  */
  ...
  async getUserByCode (options) {
    ...
    if (this.isMiniProgram) {
      let data = await this.getSessionKey(code);
      let user;
      try {
...
class API {
  ...
  constructor({componentAppId, componentAppSecret, getComponentTicket, getComponentToken, saveComponentToken, authorizerAppId, getToken, saveToken, componentApi, appId, appSecret, tokenFromCustom}) {
    ...
    this.componentAppId = componentApi.component_appid;
    this.componentAppSecret = componentApi.component_appsecret;
    this.appId = authorizerAppId || appId;
